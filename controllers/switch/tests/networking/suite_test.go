...
var _ = BeforeSuite(func() {
	...
	metalapiWebhookPath := filepath.Join("..", "..", "..", "..", "config", "webhook")
	ipamCRDPath := getExternalCrdPath(ipamv1alpha1.Subnet{})
	ipamWebhookPath := getExternalWebhookPath(ipamv1alpha1.Subnet{})
	testEnv = &envtest.Environment{
		CRDDirectoryPaths:     []string{metalapiCRDPath, ipamCRDPath},
		ErrorIfCRDPathMissing: true,
		WebhookInstallOptions: envtest.WebhookInstallOptions{
			Paths: []string{metalapiWebhookPath, ipamWebhookPath},
		},
	}
	...
	Expect((&ipamctrl.NetworkReconciler{
		Client: k8sManager.GetClient(),
		Scheme: k8sManager.GetScheme(),
		Log:    ctrl.Log.WithName("controllers").WithName("network-controller"),
	}).SetupWithManager(k8sManager)).NotTo(HaveOccurred())
	...
	Expect((&ipamctrl.SubnetReconciler{
		Client: k8sManager.GetClient(),
		Scheme: k8sManager.GetScheme(),
		Log:    ctrl.Log.WithName("controllers").WithName("subnet-controller"),
	}).SetupWithManager(k8sManager)).NotTo(HaveOccurred())
	...
	Expect((&ipamctrl.IPReconciler{
		Client: k8sManager.GetClient(),
		Scheme: k8sManager.GetScheme(),
		Log:    ctrl.Log.WithName("controllers").WithName("ip-controller"),
	}).SetupWithManager(k8sManager)).NotTo(HaveOccurred())
...
func getExternalCrdPath(crdPackageScheme interface{}) string {
...
func getExternalWebhookPath(crdPackageScheme interface{}) string {
	globalPackagePath := reflect.TypeOf(crdPackageScheme).PkgPath()
	goModData, err := os.ReadFile(filepath.Join("..", "..", "..", "..", "go.mod"))
	Expect(err).NotTo(HaveOccurred())
	goModFile, err := modfile.Parse("", goModData, nil)
	...
	Expect(globalModulePath).NotTo(BeZero())
	return filepath.Join(build.Default.GOPATH, "pkg", "mod", globalModulePath, "config", "webhook")
}
