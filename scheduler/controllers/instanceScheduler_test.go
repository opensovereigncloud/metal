...
func instanceScheduler(a *assert.Assertions) *controllers.InstanceScheduler {
	l := newLogger()
	fakeClient, err := fake.NewFakeClient()
	a.Nil(err, "must create client")
	serverExtractor := kubernetestpersictence.NewServerExtractor(fakeClient)
	instanceCleanerUseCase := scenarios.NewInstanceSchedulerUseCase(serverExtractor)
	return controllers.NewInstanceScheduler(
		l,
