...
func TestGetCrdPath(t *testing.T) {
	t.Parallel()
	expected := filepath.Join(build.Default.GOPATH, "pkg/mod/github.com/onmetal/ipam@v0.0.25/config/crd/bases")
...
func TestGetWebhookPath(t *testing.T) {
	t.Parallel()
	expected := filepath.Join(build.Default.GOPATH, "pkg/mod/github.com/onmetal/ipam@v0.0.25/config/webhook")
