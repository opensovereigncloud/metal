...
func NewManager(config ManagerConfig) (*ControllerManager, error) {
	...
	controllerManager, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "38b1cb11.onmetal.de",
	})
	if err != nil {
		return nil, err
	}
	if err := setupHealthEndpoints(controllerManager); err != nil {
		return nil, err
	}
	return &ControllerManager{Manager: controllerManager}, nil
}

func setupHealthEndpoints(controllerManager ctrl.Manager) error {
	if healthErr := controllerManager.AddHealthzCheck("healthz", healthz.Ping); healthErr != nil {
		return healthErr
	}
	if readyErr := controllerManager.AddReadyzCheck("readyz", healthz.Ping); readyErr != nil {
		return readyErr
	}
	return nil
}
