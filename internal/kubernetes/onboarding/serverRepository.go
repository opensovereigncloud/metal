...
func (s *ServerRepository) Get(request dto.Request) (dto.Server, error) {
	oob, err := s.getOOB(request)
	if err != nil {
		return dto.Server{}, err
	}
	return dto.Server{
		UUID:              oob.Status.UUID,
		PowerCapabilities: oob.Status.Capabilities,
	}, nil
}

func (s *ServerRepository) getOOB(request dto.Request) (*oobv1.OOB, error) {
	oob := &oobv1.OOB{}
	err := s.
		client.
		Get(
